...
const API_KEY = import.meta.env.VITE_OPENROUTER_API_KEY || "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";
const MODEL =
...
- Greetings like "سلام" are allowed; respond briefly and ask for the study question.
...
function extractAssistantText(data) {
	...
	if (Array.isArray(content)) {
		const combined = content.map((part) => textFromContentPart(part)).join("").trim();
		if (combined) {
			return combined;
		}
	}
...
export default function AIChat() {
	...
	const sendMessage = async () => {
		...
		try {
			...
			if (!assistantText && MODEL !== FALLBACK_MODEL) {
				const fallbackData = await requestCompletion(nextMessages, FALLBACK_MODEL);
	...
	return (
		<div className="flex flex-col w-full max-w-4xl h-144 border rounded-2xl bg-white">
